...
class MapView extends NodeView {
	constructor( currentLocationIdx ) {
		...
		this.btnBack = new ButtonView("btnDismissMap", "gfx/btn_blue.sprite", "Back", "14px Arial", "#FFFFFF");
		...
		for(var idx in g_locations) {
			...
			if( this.locationIdx == idx ) {
				node = new ButtonView( "btnDismissMap", "gfx/btn_white.sprite", loc.name, "14px Arial", "#000000", {idx:idx} );
				//node.disabled = true;
			}else {
				var canReach =  fromLoc.canReach.hasOwnProperty(loc.name);
				if(!canReach) {
					node = new ButtonView( "", "gfx/btn_dark.sprite", loc.name, "14px Arial", "#FFFFFF", {idx:idx} );
					node.alpha = 0.7;
					node.disabled = true;
				}else {
					node = new ButtonView( "mapTouch", "gfx/btn_blue.sprite", loc.name, "14px Arial", "#FFFFFF", {idx:idx} );
					node.alpha = 0.8;
				}
			}
