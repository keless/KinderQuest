...
class CastEffect {
	...
	init(originState, effectIdx, fromEntity, isChannelEffect, isPassiveEffect) {
		this.m_pParent = originState;
		this.m_pModel = originState.m_pModel;
		//for languages where this matters: this.m_pModel.retain();
		this.m_modelEffectIndex = effectIdx;
		this.m_isChannelEffect = isChannelEffect;
		this.m_isPassiveEffect = isPassiveEffect || false;
		...
		if(json["valueStat"]) {
			var casterStatValue = fromEntity.getProperty(json["valueStat"]);
			var statAddedValue =  casterStatValue * (json["valueMultiplier"] || 1.0);
			statAddedValue = Math.max(statAddedValue, 1); //minimum 1
			this.m_value += ~~(statAddedValue); //cast to int
			//console.log("value stat " + json["valueStat"] + ":" + casterStatValue + " x " + (json["valueMultiplier"] || 1.0) + " = "+ this.m_value);
		}
	...
	hasReturnEffect() {
		var json = this.getDescriptor();
		return json.hasOwnProperty("returnEffect");
	}
	...
	//int 
	getNumTicks() {
		return  ~~(this.m_lifeTime / this.m_tickFreq);
	}
	
	// in: float dt
	onTick(dt) {		
		if( !CastWorldModel.Get().isValid( this.m_pTarget ) ) {
			this.cancelTicks();
			return;
		}
		...
		var tickDelta = delta - this.m_tickFreq * this.m_numTicksCompleted;
		...
		if( this.m_type == CastEffectType.SUPPRESS_STAT || this.m_type == CastEffectType.BUFF_STAT ) {
			...
			var ticksToDo = ~~(tickDelta / this.m_tickFreq);
			var ticksLeft = this.getNumTicks() - this.m_numTicksCompleted;
			...
			ticksLeft = this.getNumTicks() - this.m_numTicksCompleted;
			if( ticksLeft <= 0) 
			{
				this.m_pTarget.removeEffect(this);
				this.cancelTicks(); 
	...
	doEffect() {
		...
		if( json.hasOwnProperty("returnEffect") ) {
			...
			bounce.initReturnEffect(this, this.m_value);

			var multiplier = json["valueMultiplier"] || 1.0;
			bounce.m_value = this.m_value * multiplier;
			//swap direction
			var f = this.m_pTarget;
			var to = this.m_pOrigin;
			
			var ghostTarget = new CastTarget();
			ghostTarget.addTargetEntity(to);
			
			world.addEffectInTransit(f, bounce, ghostTarget, CastCommandTime.Get());
	...
	getDescriptor( descriptorName ) {
		...
		}else if( this.m_isPassiveEffect ) {
			json = this.m_pModel.getPassiveEffect(this.m_modelEffectIndex);
		}else {
